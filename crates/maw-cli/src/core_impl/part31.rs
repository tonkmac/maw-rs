...
fn run_tokens_alias_command(_: &[String]) -> CliOutput {
    CliOutput {
        code: 0,
        stdout: format!("{}\n", token_format_list()),
        stderr: String::new(),
...
fn run_token_command(argv: &[String]) -> CliOutput {
    let positional = argv.iter().filter(|arg| !arg.starts_with('-')).map(String::as_str).collect::<Vec<_>>();
    let Some(sub) = positional.first().copied() else {
        return CliOutput { code: 0, stdout: format!("{}\n", token_help()), stderr: String::new() };
    };
    match sub {
        "current" => CliOutput { code: 0, stdout: token_current().map_or_else(String::new, |name| format!("{name}\n")), stderr: String::new() },
        "list" | "ls" | "tokens" => CliOutput { code: 0, stdout: format!("{}\n", token_format_list()), stderr: String::new() },
        "use" | "save" | "load" | "scan" => CliOutput { code: 2, stdout: String::new(), stderr: format!("maw token {sub}: deferred in native 4b; use maw-js until exec-backed secret vault port lands\n") },
        _ => CliOutput { code: 1, stdout: format!("{}\n", token_help()), stderr: format!("maw token: unknown subcommand \"{sub}\" (expected list|use|current|save|load|scan)\n") },
    }
}
...
fn token_current() -> Option<String> {
    ...
    detect_active_token(&content)
}

fn detect_active_token(content: &str) -> Option<String> {
    let active = content.lines().filter(|line| !line.trim_start().starts_with('#')).collect::<Vec<_>>().join("\n");
    if let Some(value) = extract_between(&active, "CLAUDE_TOKEN_NAME=\"", "\"") { return Some(value); }
    ...
    if let Some(var) = extract_after(&active, "export CLAUDE_CODE_OAUTH_TOKEN=$") {
        let needle = format!("{var}=\"$(pass show claude/token-");
        if let Some(value) = extract_between(&active, &needle, ")\"") { return Some(value); }
    }
    None
}
...
fn extract_after(haystack: &str, start: &str) -> Option<String> {
    let start_idx = haystack.find(start)? + start.len();
    let tail = &haystack[start_idx..];
