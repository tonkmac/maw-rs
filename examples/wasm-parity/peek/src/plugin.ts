import { capture, listSessions } from "../../../../packages/wasm-sdk/assembly";
...
export function handle(): i32 {
  ...
  listSessions("{}");
  if (target == "") return finish(true, "", null);
  const out = capture("{\"target\":" + quote(target) + "}");
  if (out.indexOf("\"ok\":true") < 0) return finish(false, null, "window not found: " + target);
  const value = jsonStringField(out, "content");
  return finish(true, "\u001b[36m--- " + target + " ---\u001b[0m\n" + value, null);
}
